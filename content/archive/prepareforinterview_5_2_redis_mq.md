...
>最常用的是**Cache Aside**即`写库`+`删缓存` -> `读库`+`写缓存`适配大多数场景。
>不过极端高并发情况下，确实会有极小概率导致不一致，因为（掰开了详细版）：

`写库人-w` 和 `读库人-r` 是两个线程，假如他们实际发生的顺序是：
0. r读缓存，未命中（假设这个数据很久没读了，就是没有缓存）。
1. w业务更新，开始事务写数据库，还未提交。
2. r回源读库，读到value=old，准备接下来去写缓存，这个时候网络抖动了下...
3. w事务已经提交，此时value=new，接下来去删除缓存。
4. w删除缓存成功。
5. r抖动结束，开始写缓存，把value=old写入缓存。

...
| docker-compose | 多服务组合运行配置（如 web + redis） |

**常见命令简记**：  
- 构建：`docker build -t myapp .`  
- 运行：`docker run -p 8080:80 myapp`  
- 查看日志：`docker logs myapp`

### ✅ 开发者关注点

- 开发过程中用 Docker 统一依赖环境（如 Python3.9 + Redis + MySQL）
- 提交代码后 CI 系统用 Dockerfile 构建镜像，推送到镜像仓库
- 上线使用该镜像部署，实现一致性交付

### ✅ 面试时这样说（例子）：

> “我们用 Docker 把服务容器化，每次提交代码后由 CI 自动构建镜像，然后上线环境直接拉镜像运行，保证开发和生产环境完全一致。”


## ☁️ 云原生理念与 Service Mesh（了解）
